...
class EdoStates(object):
    ...
    def change_gripper_state(self, open):
        ...
        self.msg_jca.joints = [JointControl(self.current_joint_states.joints[i].position,
                                            ...
                                            self.current_joint_states.joints[i].current, 0, 0, 0) for i in range(6)] + [JointControl(self.gripper_position, 0, 0, 0, 0)]
    ...
    def create_joint_command_message(self, joint_names, point):
        ...
        # add one zero at the end cause the message JointControl changed in version 3.0 of edo
        self.msg_jca.joints = [JointControl(point.positions[i]/0.01745, point.velocities[i]/0.01745, 0, 0, 0, 0) for i in range(6)] + [JointControl(self.gripper_position, 0, 0, 0, 0)]
        return self.msg_jca
